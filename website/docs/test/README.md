...
[suite](https://github.com/greglv93/Antibody-Annotation-to-JSON/tree/feature/docusaurus-site/test/schema_tests) of manually written valid and invalid examples. In addition, the JSON schema is linted and this 
...
[one complete manually written expected output file](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/test/expected_json_files/12023_expected.json) from one of the 
more complicated examples of real input data. This file is used both within the Python unit test suite (although it's 
closer to a full 'integration test') and as a [test case](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/test/schema_tests/full_schema.json) within the schema tests.
...
[pre-commit hooks](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/.pre-commit-config.yaml) should be temporarily disabled (e.g. by commenting them out or moving 
them to the [optional hooks](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/.pre-commit-optional.yaml)). The tests should still be run manually to inform the 
...
(at least initially), disable any [pre-commit hooks](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/.pre-commit-config.yaml) that get in the way of a smooth and 
frequent enough edit and commit cycle. The hook definitions can be moved to the 
[optional config](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/.pre-commit-optional.yaml), where they can still be run manually with 
...
For testing the parser locally with new or modified input data, use the [/input_data/](https://github.com/greglv93/Antibody-Annotation-to-JSON/tree/feature/docusaurus-site/input_data) and 
[/json_files/](https://github.com/greglv93/Antibody-Annotation-to-JSON/tree/feature/docusaurus-site/json_files) directories as these are ignored by git, while the equivalent subdirectories within the 
...
Each file in the [schema test suite](https://github.com/greglv93/Antibody-Annotation-to-JSON/tree/feature/docusaurus-site/test/schema_tests) contains at least one valid example followed by several invalid 
...
In addition, there is a [full schema test case](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/test/schema_tests/full_schema.json) that imports a 
['golden example'](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/test/expected_json_files/12023_expected.json) of what the converted JSON output should look like for the 
...
a [helper script](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/antibody_annotation_to_json/dev/fix_schema_targets.py) that re-writes the test files with valid 
"target" URI strings. This script is called in a [pre-commit hook entry](https://github.com/greglv93/Antibody-Annotation-to-JSON/blob/feature/docusaurus-site/.pre-commit-config.yaml), which blocks a
